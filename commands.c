    ...
    void *arg)
{
	...
	if (new == NULL) {
		log_warn("mgmtctl", "unable to allocate memory for new command node");
		return NULL;
	}
	new->token = token;
	new->doc = doc;
    ...
    const char *key, const char *value)
{
	...
	if (el == NULL) {
		log_warn("mgmtctl", "unable to allocate memory for new environment variable");
		return -1;
	}
	el->key = key;
	el->value = value;
...
cmdenv_pop(struct cmd_env *env, int n)
{
	while (n-- > 0) {
		if (TAILQ_EMPTY(&env->stack)) {
			log_warnx("mgmtctl", "environment stack is empty");
			return -1;
		}
...
cmdenv_push(struct cmd_env *env, struct cmd_node *node)
{
	...
	if (el == NULL) {
		log_warn("mgmtctl", "not enough memory to allocate a stack element");
		return -1;
	}
	el->el = node;
 ...
 *
 * @param conn    Connection to mgmtd.
    ...
    char **word, int all, int priv)
{
	...
	if (!completion)
		for (n = 0; n < argc; n++)
			log_debug("mgmtctl", "argument %02d: `%s`", n, argv[n]);
...
#define CAN_EXECUTE(candidate) \
	...
	while ((current = cmdenv_top(&env))) {
		...
		if (token == NULL ||
			...
			goto end;
		if (!completion)
			log_debug("mgmtctl", "process argument %02d: `%s`",
			    env.argp, token);
		TAILQ_FOREACH(candidate, &current->subentries, next) {
			if (candidate->token &&
				...
				if (!best) best = candidate;
					...
					if (!completion)
						log_warnx("mgmtctl", "ambiguous token: %s (%s or %s)",
		...
		if (!best) {
			if (!completion)
				log_warnx("mgmtctl", "unknown command from argument %d: `%s`",
				    env.argp + 1, token);
			rc = -1;
			goto end;
		}
	...
	if (!completion && !help && !complete) {
		if (rc == 0 && env.argp != env.argc + 1) {
			log_warnx("mgmtctl", "incomplete command");
			rc = -1;
		}
